        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(textRenderer.valueText(value, metric: config.metric))
                        .font(.system(size: 34, weight: .semibold))
                        .foregroundStyle(AppTheme.Colors.primaryText)

                    Text(
                        L10n.text(
                            "growth.ruler.instruction",
                            en: "Swipe horizontally to align the value with the center marker.",
                            zh: "横向滑动，让数值对齐中央准星"
                        )
                    )
                ...
                Button(action: onTapManualInput) {
                    Text(L10n.text("growth.ruler.manual_input", en: "Manual Input", zh: "⌨️ 手动输入"))
