...
final class GrowthStore {
    ...
    init(
        ...
        formatter: GrowthFormatter = GrowthFormatter(),
...
extension GrowthStore {
    ...
    private func refreshCurrentMetric() {
        ...
        guard let repository else {
            ...
            viewState.errorMessage = "成长数据暂不可用"
            return
        }
        ...
        do {
            ...
            viewState.dataState = .error
            viewState.errorMessage = "成长数据加载失败"
    ...
    private func prepareEntryDraft(for metric: GrowthMetric) {
        ...
        viewState.entryDraft.manualInput = formatter.formatEditableValue(viewState.entryDraft.value)
    }
    ...
    private func saveRecord() {
        ...
        do {
            ...
            viewState.sheetState = .closed
            refreshCurrentMetric()
            showUndoToast(recordID: record.id, message: "已记录\(formatter.formatValue(value, metric: metric))")
            AppHaptics.success()
        } catch {
    ...
    private func currentValidatedDraftValue() -> Double? {
        ...
        if viewState.sheetState.isManualInput {
            guard let value = Double(viewState.entryDraft.manualInput) else { return nil }
