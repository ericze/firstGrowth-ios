...
final class HomeStore {
    ...
    @ObservationIgnored private let commonFoodTags = ["米粉", "南瓜", "香蕉", "鸡蛋", "牛油果", "面条", "豆腐", "胡萝卜", "土豆", "西兰花", "第一次尝试"]

    init(
        headerConfig: HomeHeaderConfig,
        recordRepository: RecordRepository? = nil,
        formatter: TimelineContentFormatter = TimelineContentFormatter(),
...
extension HomeStore {
    ...
    var suggestedFoodTags: [String] {
        commonFoodTags.filter { !recentFoodTags.contains($0) }
    }
    ...
    func handle(_ action: HomeAction) {
        ...
        case let .selectModule(module):
            guard routeState.currentModule != module else { return }
            routeState.currentModule = module
    ...
    private func saveDiaper(subtype: DiaperSubtype) {
        ...
        do {
            ...
            routeState.activeSheet = nil
            integrateCreatedRecord(record, message: "已记录\(subtype.title)")
            AppHaptics.mediumImpact()
        } catch {
    ...
    private func finishSleep() {
        ...
        do {
            ...
            routeState.activeSheet = nil
            integrateCreatedRecord(record, message: "已结束睡眠")
            AppHaptics.success()
        } catch {
    ...
    private func saveFood() {
        ...
        do {
            ...
            routeState.activeSheet = nil
            integrateCreatedRecord(record, message: "已记录一顿辅食")
            AppHaptics.mediumImpact()
        } catch {
    ...
    private func loadMoreHistoryIfNeeded(for recordID: UUID) {
        guard
            routeState.currentModule == .record,
