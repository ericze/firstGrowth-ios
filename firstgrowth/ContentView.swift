...
struct ContentView: View {
    ...
    var body: some View {
        HomeContainerView(store: store, growthStore: growthStore, treasureStore: treasureStore)
            ...
            .task {
                ...
                if let initialModule = launchOverrides.initialModule {
                    store.handle(.selectModule(initialModule))
                }
...
private struct AppLaunchOverrides {
    ...
    init(environment: [String: String] = ProcessInfo.processInfo.environment) {
        initialModule = environment["FG_INITIAL_MODULE"].flatMap(HomeModule.init(rawValue:))
        initialGrowthMetric = environment["FG_GROWTH_METRIC"].flatMap(GrowthMetric.init(rawValue:))
        growthSeedPreset = environment["FG_GROWTH_SEED"].flatMap(GrowthSeedPreset.init(rawValue:))
        treasureSeedPreset = environment["FG_TREASURE_SEED"].flatMap(TreasureSeedPreset.init(rawValue:))
        opensGrowthEntry = environment["FG_GROWTH_OPEN_ENTRY"] == "1"
        growthEntryMode = environment["FG_GROWTH_ENTRY_MODE"].flatMap(GrowthEntryLaunchMode.init(rawValue:)) ?? .ruler
    }
